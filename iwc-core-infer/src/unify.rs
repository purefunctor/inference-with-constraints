...
impl super::Infer {
    ...
    fn emit_deep(&mut self, t_name: usize, u_name: usize) {
        self.constraints.push(Constraint::UnifyDeep(t_name, u_name))
    }

    fn emit_solve(&mut self, t_name: usize, u_idx: TypeIdx) {
        self.constraints.push(Constraint::UnifySolve(t_name, u_idx))
    }

    fn emit_error(&mut self, error: UnifyError) {
        self.constraints.push(Constraint::UnifyError(error))
    }
}
